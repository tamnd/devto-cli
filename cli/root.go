// Package cli assembles the devto command tree from the devto
...
// NewApp assembles the kit application from the devto domain. The
...
//
// To add a command, declare it in devto/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := devto.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(devto.Domain{}).Register(app)
