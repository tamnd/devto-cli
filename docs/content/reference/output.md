...
describes what users get. Pick a format with `-o`, or let devto choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
devto <command> -o table   # aligned columns for reading
devto <command> -o jsonl   # one JSON object per line, for piping
devto <command> -o json    # a single JSON array
devto <command> -o csv     # spreadsheet friendly
devto <command> -o tsv     # tab-separated
devto <command> -o url     # just the URL column
devto <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
devto <command> --fields id,title,url
```
...
```bash
devto <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
devto <command>            # a table, because this is a terminal
devto <command> | wc -l    # JSONL, because this is a pipe
```
